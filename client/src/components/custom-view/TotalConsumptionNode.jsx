...
const TotalConsumptionNode = ({ data, selected }) => {
    const [value, setValue] = useState(0);
    const [prevValue, setPrevValue] = useState(null);
    const [loading, setLoading] = useState(false);

    // Helper to calculate start/end dates (Copied from Dashboard.jsx)
        ...
        <>
            ...
            <div style={style}>
                <div className="flex flex-col items-center gap-1 z-10 p-2 text-center">
                    <Zap size={24} className="text-white/90" />
                    ...
                    <div className="text-[10px] opacity-80 uppercase tracking-wider text-slate-700">
                        Consumption ({data.timeRange || 'Day'})
                    </div>
                </div>
            </div>
        </>
...
export default memo(TotalConsumptionNode);
