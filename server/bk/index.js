...
app.get('/api/meters', authenticateToken, async (req, res) => {
    try {
        ...
        // Create lookup maps
        const displayNameMap = new Map(); // c_serial_id (stripped) -> c_name

        webSerialData.forEach(r => {
            const serialId = String(r.c_serial_id);
            const strippedSerial = serialId.includes('\\') ? serialId.split('\\').pop() : serialId;
            displayNameMap.set(strippedSerial.toUpperCase(), r.c_name || strippedSerial);
        });
        ...
        const filteredMeters = allMeters.recordset.filter(m => {
            const meterVal = String(m.val || '').toUpperCase();
            const meterSerial = String(m.serial || '').toUpperCase();
            ...
            return allowedSerialsSet.has(meterVal) ||
                allowedSerialsSet.has(meterSerial) ||
                allowedSerialsSet.has(cleanVal) ||
                allowedSerialsSet.has(cleanSerial);
        }).map(m => {
            const meterSerial = String(m.serial || m.val || '');
            const cleanSerial = meterSerial.includes('\\') ? meterSerial.split('\\').pop() : meterSerial;
            const displayName = displayNameMap.get(cleanSerial.toUpperCase()) || cleanSerial;
    ...
    try {
        ...
        if (paraChart === 'Report') {
            ...
            const pivotColumns = columns.map(col =>
                `ISNULL(MAX(CASE WHEN c_column = '${col}' THEN n_unit END), 0) as [${col}]`
            ).join(', ');
...
app.get('/api/smartboards/:id', requirePermission(39), async (req, res) => {
    try {
        ...
        const sql = `
            select boardid,active,command,mtype,address 
            from dbo.smartboard 
            where boardid like '${id}%' 
            order by case when left(boardid,1)='I' then iif(active='B',0, iif(active='Y',1, 2) ) else 9 end, boardid
        `;
...
app.get('/api/settings/available-serials', requirePermission(34), async (req, res) => {
    try {
        ...
        if (isSuperGroup) {
            const result = await req.db.request()
                .query("SELECT * FROM WebSerial ORDER BY c_serial_id");
            return res.json(result.recordset);
        }

        // Non-super-group: Get serials from WebMainSub hierarchy
        let sqlQuery = `
            SELECT s.* FROM WebSerial s
            WHERE s.c_id IN (
                SELECT ws.sub_id FROM WebMainSub ws
        ...
        // If Site User, restrict to serials under THEIR site only
        if (userLevel === 'SITE') {
            sqlQuery = `
                SELECT s.* FROM WebSerial s
                WHERE s.c_id IN (
        ...
        sqlQuery += ` ORDER BY s.c_serial_id`;
...
// [NEW] Create New Site
app.post('/api/settings/sites', authenticateToken, async (req, res) => {
    try {
        const { name, active } = req.body;
        const userId = req.user.id;
        ...
        // Generate new site ID (format: 1XXXXXXXXX for sites)
        const maxIdResult = await req.db.request()
            .query("SELECT MAX(CAST(c_id AS BIGINT)) as maxId FROM WebSite WHERE c_id LIKE '1%'");

        let newId;
        if (maxIdResult.recordset[0].maxId) {
            newId = (BigInt(maxIdResult.recordset[0].maxId) + 1n).toString();
        } else {
            newId = '100000001';
        }

        const activeValue = active ? 'Y' : 'N';
        const now = new Date().toISOString();

        await req.db.request()
            .input('id', sql.VarChar, newId)
            .input('name', sql.NVarChar, name.trim())
            .input('active', sql.VarChar, activeValue)
            .input('date', sql.VarChar, now.substring(0, 10))
            .query(`
                INSERT INTO WebSite (c_id, c_name, c_active, c_date)
                VALUES (@id, @name, @active, @date)
            `);

        res.json({ success: true, siteId: newId, message: 'Site created successfully' });
    } catch (err) {
        ...
        try {
            ...
            if (serialIds && serialIds.length > 0) {
                for (const serialId of serialIds) {
                    await transaction.request()
                        .input('sid', sql.VarChar, siteId)
                        .input('ser', sql.VarChar, serialId)
