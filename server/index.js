...
// ===== Background workers (MQTT notifier + webpush dispatcher) =============
// Two ways to run the background workers:
//
//   1. SEPARATE process (default, recommended) — run `node worker.js` in its
//      own terminal / Windows Service. This API server only serves HTTP and
//      doesn't touch MQTT or push delivery. Restarting the API doesn't
//      disconnect MQTT.
//   2. IN-PROCESS (legacy) — set ENABLE_MQTT_WORKER=true in .env to boot the
//      workers inside this Node process. Use when you want one process for
//      everything (simpler ops, but every restart drops the MQTT session).
//
// ⚠️ Don't enable both modes at once — the dispatcher would run twice and
